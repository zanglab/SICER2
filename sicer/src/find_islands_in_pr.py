...
def filter_ineligible_windows(chrom_graph, min_tags_in_window, average):
    '''
    Filters windows that have tag count lower than the minimum threshold count and calculates score for windows that
...
def main(args, total_read_count, pool):
    ...
    s_logger.info(f"Species: {args.species}")
    s_logger.info(f"Window_size: {args.window_size}")
    s_logger.info(f"Gap size: {args.gap_size}")
    s_logger.info(f"E value is: {args.e_value}")
    s_logger.info(f"Total read count: {total_read_count}")
    chroms = GenomeData.species_chroms[args.species]  # list of chromsomes for the given species (e.g. chr1, ... , chrx)
    genome_length = sum(GenomeData.species_chrom_lengths[args.species].values())  # list of length of each chromosomes
    ...
    s_logger.info(f"Genome Length: {genome_length}")
    s_logger.info(f"Effective genome Length: {effective_genome_length}")
    s_logger.info(f"Window average: {average}")

    window_pvalue = 0.20
    bin_size = 0.001
    background = Background_island_probscore_statistics.Background_island_probscore_statistics(
        total_tags=total_read_count,
        windowSize=args.window_size,
        gapSize=args.gap_size,
        window_pvalue=window_pvalue,
        genomeLength=effective_genome_length,
        bin_size=bin_size)
    min_tags_in_window = background.min_tags_in_window

    s_logger.info(f"Window pvalue: {window_pvalue}")
    s_logger.info(f"Minimum num of tags in a qualified window: {min_tags_in_window}")  # first threshold cutoff
    s_logger.info(' ')
    logger.run("Determine the score threshold from random background.")
    # determine threshold from random background
    score_threshold = background.find_island_threshold(args.e_value)
    s_logger.info(f"The score threshold is: {score_threshold}")
    ...
    s_logger.info(' ')
    logger.run("Generate the enriched probscore summary graph and filtering the summary graph "
               "to eliminate ineligible windows.")
    ...
    s_logger.info(f"Total number of islands: {total_number_islands}")
